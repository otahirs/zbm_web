...
{% if (authorize(['site.blizise'])) %} {# kod mezi if se zobrazi jen pro uzivatele autorizovane skupiny #}
  ...
  $(".editBliziSeButton").click(function(){
      ...
      content.innerHTML = '<form method="post" action="/php/blizise">' +  //nahradi text blizi se formularem na upravu
                            ...
                            '<button class="saveBlizise special" type="submit" style="margin-right:5px; margin-top: 1em">Uložit</button><br>' +
                            '<div class="pure-g">' +
                            '<div class="pure-u-1-2">' +
                            '<button class="regenerateBliziSe fit small" type="button" margin-top: 1em"><i class="fa fa-refresh" aria-hidden="true"></i></button>' +
                            '</div>' +
                            '<div class="pure-u-1-2">' +
                            '<button class="editBliziSeCancel fit small" style="margin-top: 1em">Zrušit</button>' +
                            '</div>' +
                            '</div>' +
                            '</form>';
